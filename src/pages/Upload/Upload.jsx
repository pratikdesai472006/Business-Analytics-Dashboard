...
function Upload() {
  ...
  const pick = (selectedFile) => {
    ...
    if (selectedFile.size > 4.5 * 1024 * 1024) {
      setFile({ error: "File size exceeds Vercel serverless limit of 4.5 MB. Please upload a smaller CSV file." });
      return;
    }
  ...
  const activateDataset = async (dataset) => {
    try {
      await api.patch(`/datasets/${dataset.id}/activate`, {}, auth);
      setHistory((current) =>
        current.map((item) => ({ ...item, isActive: item.id === dataset.id }))
      );
      setNotice(`Activated dataset: ${dataset.name}`);
      window.dispatchEvent(new CustomEvent("dataset:updated"));
    } catch {
      setError("Could not activate this dataset.");
    }
  };
