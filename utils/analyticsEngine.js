...
const buildInsights = (rows, summary) => {
  const insights = [];
  const revenueValues = rows.map((row) => normalizeValue(pickValue(row, ["revenue", "Revenue", "amount", "total", "totalRevenue"])));
  ...
  const products = rows.reduce((acc, row) => {
    ...
    const revenue = normalizeValue(pickValue(row, ["revenue", "Revenue", "amount", "total", "totalRevenue"]));
  ...
  const monthlyRevenueMap = rows.reduce((acc, row) => {
    ...
    const value = normalizeValue(pickValue(row, ["revenue", "Revenue", "amount", "total", "totalRevenue"]));
  ...
  if (orderCount) {
    insights.push({ tone: "blue", title: "Order volume", detail: `You recorded ${orderCount} orders with an average order value of ₹${averageOrderValue.toLocaleString("en-IN")}.` });
  ...
  if (topProduct) insights.push({ tone: "green", title: "Best-selling product", detail: `${topProduct.name} drove ₹${topProduct.revenue.toLocaleString("en-IN")} in revenue.` });
  if (bottomProduct && bottomProduct.name !== topProduct?.name) insights.push({ tone: "amber", title: "Watchlist product", detail: `${bottomProduct.name} is the lowest-revenue product in this dataset.` });
  if (topCustomer) insights.push({ tone: "violet", title: "Top customer", detail: `${topCustomer.name} generated ₹${topCustomer.revenue.toLocaleString("en-IN")}.` });
  if (uniqueCustomers) insights.push({ tone: "blue", title: "Repeat customer rate", detail: `${repeatRate.toFixed(1)}% of customers placed more than one order.` });
  ...
  if (momentum < -10) insights.push({ tone: "amber", title: "Revenue drop", detail: "Revenue has fallen sharply compared with the previous period." });
  if (cancelled > 0 && (cancelled / Math.max(1, orderCount)) > 0.15) insights.push({ tone: "amber", title: "High cancellation risk", detail: "Cancelled orders are elevated; monitor conversion quality." });
  if (returned > 0 && (returned / Math.max(1, orderCount)) > 0.1) insights.push({ tone: "amber", title: "Return concern", detail: "Returns are increasing; inspect fulfilment and product issues." });
  if (summary?.activeCustomers && summary.activeCustomers > 0) insights.push({ tone: "blue", title: "Customer concentration", detail: `${summary.activeCustomers} active customers are driving the current revenue mix.` });
  if (orderCount && expectedNextOrders > 0) insights.push({ tone: "green", title: "Future order outlook", detail: `The next cycle is expected to bring about ${expectedNextOrders} orders.` });
...
const buildAnalytics = (dataset, rows) => {
  ...
  const summary = {
    totalRevenue,
    totalOrders: orders,
    activeCustomers: new Set(rows.map((row) => String(pickValue(row, ["customer", "Customer", "customerName", "customer_name"]) || "").trim()).filter(Boolean)).size,
    growth,
